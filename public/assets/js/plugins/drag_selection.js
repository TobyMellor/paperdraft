(function() {
    ...
    function getActiveObjectsInsideSelection()
    {
        for(var i = 0; i < activeObjects.length; i++) {
            if(activeObjects[i]['object_position_x'] >= startX / 32
                    && activeObjects[i]['object_position_x'] <= endX
                    && activeObjects[i]['object_position_y'] >= startY / 32
                    && activeObjects[i]['object_position_y'] <= endY) {
                console.log(activeObjects[i]['object_position_x'] + ', ' + activeObjects[i]['object_position_y']);
            }
        }
    }
    ...
    document.onmouseup = function(event) {
        ...
        endX = (getCoords(event)['x'] - $('.drop-target').offset()['left']) / 32;
        endY = (getCoords(event)['y'] - $('.drop-target').offset()['top']) / 32;
